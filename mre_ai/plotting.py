...
from mre_ai.preprocessing import MRIImage
from mre_ai.pytorch_arch_deeplab_3d import DeepLabFeatures
from mre_ai.mre_datasets import MRETorchDataset
...
def xr_viewer_v2(xr_ds, grid_coords=None, group_coords=None,
    ...
    print(xr_ds)
    wave_names = list(filter(lambda x: 'wave' in x, xr_ds.mre_type.values))
    if prediction:
        hv_ds_mre_1 = hv_ds_mre.select(mre_type=['mre', 'mre_mask', 'mre_pred'])
        if len(wave_names) > 0:
            hv_ds_mre_2 = hv_ds_mre.select(mre_type=['mre_raw', wave_names[0]])
        else:
            hv_ds_mre_2 = hv_ds_mre.select(mre_type=['mre_raw'])
    elif not torch:
...
def xr_viewer_models(xr_ds, size=250, do_cor=False):
    ...
    mre_types = [mre for mre in xr_ds.mre_type.values
                 if mre not in ['mre_raw', 'mre', 'mre_pred', 'mre_wave']]
...
def confusion_plots(df):
    ...
    df['true_cat_coarse'] = pd.cut(df.mre, bins=[0, 3770, 20000], labels=False)
    df['pred_cat_coarse'] = pd.cut(df.pred, bins=[0, 3770, 20000], labels=False)
    ...
    ax[0].axvline(x=3, color='lightblue', linewidth=3)
    ax[0].axhline(y=3, color='lightblue', linewidth=3)
    ...
    ax[1].set_xticklabels(['F0-F2', 'F3-F4'], size=14)
    ax[1].set_yticks([0.5, 1.5])
    ax[1].set_yticklabels(['F0-F2', 'F3-F4'], size=14)
    fig.text(0.5, 0.04, 'Predicted', ha='center', size=18)

    fig.suptitle('Confusion Matrices (Normalized)', size=18)

def bias_plots(df_all):
    # Stratify items
    df_bias_hists = df_all[['residuals', 'mre', 'age', 'sex', 'bmi']].copy()
    df_strat = df_all[['residuals', 'mre', 'age', 'sex', 'bmi']].copy()
    columns = ['mre', 'age', 'sex', 'bmi']
    for item in columns:
        if item != 'residuals':
            if df_bias_hists[item].nunique() == 2:
                pass
            elif item=='age':
                df_bias_hists['Age'] = pd.cut(df_bias_hists[item], [17, 45, 60, 80]).astype(str)
                df_strat['Age'] = pd.cut(df_bias_hists[item], [17, 45, 60, 80]).cat.codes
            elif item=='mre':
                df_bias_hists['Stiffness'] = pd.cut(df_bias_hists[item], [1, 2.880, 4.090, 15.000]).astype(str)
                df_strat['Stiffness'] = pd.cut(df_bias_hists[item], [1.000, 2.880, 4.090, 15.000]).cat.codes
            elif item=='bmi':
                df_bias_hists['BMI'] = pd.cut(df_bias_hists[item], [18.5, 25, 30, 50]).astype(str)
                df_strat[f'BMI'] = pd.cut(df_bias_hists[item], [18.5, 25, 30, 50]).cat.codes

    # Make Plots
    sns.set()
    sns.set_palette(sns.color_palette('colorblind'))

    fig, ax = plt.subplots(2,2, figsize=(15,12), sharex=False, sharey=True)
    fig.patch.set_alpha(1)
    plt.xlabel('True Average kPa')
    ax = np.ravel(ax)
    mean = df_bias_hists.residuals.mean()
    std = df_bias_hists.residuals.std()

    for i, group in enumerate(['Stiffness', 'BMI', 'Age', 'sex']):
        df_anova = anova(data=df_strat, dv='residuals', between=group)
        pval = df_anova[df_anova['Source']==group]['p-unc'].values[0]
        if pval<0.001:
            pval = '<0.001'
        else:
            pval = f'{pval:.2f}'
        ax[i].plot([], [], ' ', label=f"p-value: {pval}")
        if group == 'sex':
            sns.violinplot(ax=ax[i], x='sex', y='residuals', data=df_bias_hists,)
            ax[i].set_xlabel('Sex', fontsize=16)
            ax[i].set_ylabel(None)
            ax[i].annotate("D)", xy=(-0.03, 1.04), xycoords="axes fraction", fontsize=16)
        elif group =='Stiffness':
            for j in sorted(df_bias_hists[group].unique()):
                df_tmp = df_bias_hists.query(f'{group}=="{j}"')
                ax[i].scatter(x=df_tmp['mre'], y=df_tmp['residuals'], label=j)
                # ax[i].vlines(0, 0, ax[i].get_ylim()[1], linestyle='dashed')
            ax[i].set_xlabel('True Mean Stiffness (kPa)', fontsize=16)
            ax[i].annotate("A)", xy=(-0.03, 1.04), xycoords="axes fraction", fontsize=16)
        elif group =='BMI':
            for j in sorted(df_bias_hists[group].unique()):
                df_tmp = df_bias_hists.query(f'{group}=="{j}"')
                ax[i].scatter(x=df_tmp['bmi'], y=df_tmp['residuals'], label=j)
            ax[i].set_xlabel('BMI', fontsize=16)
            ax[i].annotate("B)", xy=(-0.03, 1.04), xycoords="axes fraction", fontsize=16)
        elif group =='Age':
            for j in sorted(df_bias_hists[group].unique()):
                df_tmp = df_bias_hists.query(f'{group}=="{j}"')
                ax[i].scatter(x=df_tmp['age'], y=df_tmp['residuals'], label=j)
            ax[i].set_xlabel('Age', fontsize=16)
            ax[i].annotate("C)", xy=(-0.03, 1.04), xycoords="axes fraction", fontsize=16)

        ax[i].set_ylim(-12,12)
        ax[i].legend(fontsize=12, loc='upper left')
        if i in [0,2]:
            ax[i].set_ylabel('Residuals (kPa)', fontsize=16)
        ax[i].axhline(y=mean, color='k')
        ax[i].axhline(y=mean+std*1.96, color='k', ls='dashed')
        ax[i].axhline(y=mean-(std*1.96), color='k', ls='dashed')
