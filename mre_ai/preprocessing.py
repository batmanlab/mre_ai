...
def dicom_to_nifti(data_path, subdirs, output_path=None):
    '''Code for determining which dicom to keep, and then save it as a nifti.
    Iterates through a set of subdirs for a given data_path.'''
    ...
    for subdir in tqdm_notebook(subdirs, desc='subdir'):
        ...
        for patient in tqdm_notebook(list(semi_path.iterdir()), desc='patient'):
            ...
            for i, img_files in enumerate(img_folders):
                ...
                if output_path == None:
                    patient_path = Path(Path().absolute().parent, 'data', 'NIFTI', pid)
                    patient_path.mkdir(parents=True, exist_ok=True)
                else:
                    patient_path = Path(output_path, 'NIFTI', pid)
                    patient_path.mkdir(parents=True, exist_ok=True)
