...
from mre_ai.mre_datasets import MREtoXr, MRETorchDataset
from mre_ai.prediction import train_model, add_val_linear_cor
from robust_loss_pytorch import adaptive
from mre_ai.pytorch_arch_deeplab_3d import  DeepLab
from mre_ai.pytorch_arch_debug import Debug
from mre_ai.pytorch_arch_clinical import Clinical
...
def train_model_full(data_path: str, data_file: str, output_path: str, model_version: str = 'tmp',
    ...
    if cfg['model_arch'] != 'clinical':
        ...
        # set to number of clinical channels, hardcoded for now
        in_channels = 14
    ...
    if cfg['model_arch'] == 'base':
        ...
        print(cfg['norm'])
        model = DeepLab(in_channels=in_channels, out_channels=cfg['out_channels'],
                        output_stride=8, norm=cfg['norm'],
                        do_clinical=cfg['do_clinical'], class_only=cfg['class_only'])

    elif cfg['model_arch'] == 'clinical':
        model = Clinical(in_channels=in_channels, out_channels=cfg['out_channels'])

    elif cfg['model_arch'] == 'debug':
        model = Debug(in_channels=in_channels, out_channels=cfg['out_channels'])
    ...
    if cfg['dry_run']:
        ...
        print('Model Summary:')
        if cfg['do_clinical']:
            summary(model, input_size=[(in_channels, 32, 256, 256), list(clinical.shape[1:])])
        ...
        model, best_loss, ds_mem = train_model(model, optimizer, exp_lr_scheduler, device,
                                               ...
                                               do_val=cfg['do_val'], ds=ds,
                                               do_clinical=cfg['do_clinical'],
                                               class_only=cfg['class_only'])
...
def default_cfg():
    cfg = {'train_aug': True, 'train_sample': 'shuffle',
           ...
           'do_val': True, 'norm': 'bn', 'weight_decay': 0.1,
